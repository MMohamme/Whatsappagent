...
if not defined PYTHON_EXE set "PYTHON_EXE=%USERPROFILE%\anaconda3\python.exe"
...
if exist "%PYTHON_EXE%" (
    echo Starte FastAPI Backend mit direktem Python...
    start "WhatsApp Backend v3" cmd /k "cd /d ""%PROJECT_ROOT%"" && ""%PYTHON_EXE%"" -m uvicorn backend.main:app --host %BACKEND_HOST% --port %BACKEND_PORT%"
) else (
    echo Python nicht gefunden unter:
    echo %PYTHON_EXE%
    echo.
    echo Versuche Conda-Fallback...
    if not exist "%CONDA_ACTIVATE%" (
        echo FEHLER: Weder PYTHON_EXE noch Conda activate.bat gefunden.
        echo Setze PYTHON_EXE in .env, z.B.:
        echo PYTHON_EXE=C:\Users\dein-user\anaconda3\python.exe
        pause
        exit /b 1
    )
    start "WhatsApp Backend v3" cmd /k "cd /d ""%PROJECT_ROOT%"" && call ""%CONDA_ACTIVATE%"" %CONDA_ENV% && python -m uvicorn backend.main:app --host %BACKEND_HOST% --port %BACKEND_PORT%"
)
